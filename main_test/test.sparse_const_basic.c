...
static int dump_optimize_pass_case(void)
{
	target t;
	datadesc_set sc;
	block ob;
	...
	ob = make_sparse_block(make_direct_real(K_float, 4),
	                       make_runtime_real("input_x", 4),
	...
	if (optimized == ob)
	{
		fprintf(stderr, "optimize_sparse_const did not rewrite test case\n");
		return 1;
	}
	...
	fp = fopen("dump_sparse_opt_pass.c", "wt");
	if (!fp)
	{
		fprintf(stderr, "failed to open dump_sparse_opt_pass.c\n");
		return 1;
	}
